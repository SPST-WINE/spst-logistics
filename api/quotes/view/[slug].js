...
function buildHtml({ quote, options, packages }) {
  ...
  const pkgRows = packages.map(p=>{
    ...
    return `<tr><td style="padding:6px 8px">${qty}</td><td style="padding:6px 8px">${dims}</td><td style="padding:6px 8px">${kg.toFixed(2)}</td></tr>`;
  }).join('');
...
export default async function handler(req, res) {
  try {
    ...
    const quote = {
      ...
      shipmentNotes: pick(f, ['Note generiche sulla spedizione','Note_Spedizione','Shipment_Notes','Note spedizione'], ''),
      stato        : pick(f, ['Stato'], ''),
      acceptedAt   : pick(f, ['Accettato_Il'], ''),
      acceptedIndex: toNum(pick(f, ['Opzione_Accettata'], undefined), undefined),
      sender: {
        ...
        tax    : pick(f, ['Mittente_Tax','PIVA','P.IVA','Mittente_PIVA','Mittente_EORI','Sender_Tax']),
      },
      recipient: {
        ...
        tax    : pick(f, ['Destinatario_Tax','TaxID','EORI','Destinatario_EIN','Recipient_Tax']),
      }
    };
