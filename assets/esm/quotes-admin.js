...
function buildPreviewHtml(model){
  ...
  const optRows = (options.length ? options : []).map(o => ([
    ...
    '</div>'
  ].join(''))).join('') || '<div class="small">Nessuna opzione completa.</div>';
  ...
  const pkgRows = packages.map(p => {
    const dims = [p.l ?? p.length, p.w ?? p.width, p.h ?? p.height]
      .map(n => (Number(n)||0).toFixed(1)).join(' × ');
    const kg = (Number(p.kg ?? p.weight) || 0).toFixed(2);
    const qty = Number(p.qty) || 1;
    return '<tr><td style="padding:6px 8px">'+qty+'</td><td style="padding:6px 8px">'+dims+'</td><td style="padding:6px 8px">'+kg+'</td></tr>';
  }).join('');
