...
function buildPreviewHtml(model){
  ...
  const best = getBestIndex(options) || (options[0] ? options[0].index : null);

  // Opzioni → blocchi HTML concatenati (niente backtick annidati)
  const optRows = options.map(function(o){
    return ''
